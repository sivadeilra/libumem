...
void vmem_stand_init(void) {
  stand_chunksize = MAX(DEF_CHUNKSIZE, pagesize);

  stand_nregions = 0;
}

int vmem_stand_add(caddr_t base, size_t len) {
  ...
  sr->sr_left =
      P2ALIGN(len - (size_t)(sr->sr_curtop - sr->sr_base), stand_chunksize);
  membar_producer();
...
static void *stand_parent_alloc(vmem_t *src, size_t size, int vmflags) {
  int old_errno = errno;
  stand_region_t *sr;
  size_t chksize;
  void *ret;
  ...
  if ((ret = _vmem_extend_alloc(src, sr->sr_curtop, chksize, size, vmflags)) ==
      NULL) {
    errno = old_errno;
    return (NULL);
  }
...
vmem_t *vmem_stand_arena(vmem_alloc_t **a_out, vmem_free_t **f_out) {
  ASSERT(stand_nregions == 1);

  stand_heap =
      vmem_init("stand_parent", stand_chunksize, stand_parent_alloc, vmem_free,
