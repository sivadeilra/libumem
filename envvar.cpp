...
static umem_env_item_t umem_options_items[] = {
    ...
    {"backend", "Evolving", ITEM_SPECIAL,
     "=sbrk for sbrk(2), =mmap for mmap(2)", NULL, 0, NULL, NULL,
     &umem_backend_process},
    {"allocator", "Evolving", ITEM_SPECIAL, "=best, =first, =next, or =instant",
     NULL, 0, NULL, NULL, &umem_allocator_process},
#endif

    {"concurrency", "Private", ITEM_UINT, "Max concurrency", NULL, 0,
     &umem_max_ncpus},
    ...
    {"nomagazines", "Private", ITEM_FLAG,
     "no caches will be multithreaded, and no caching will occur.", &umem_flags,
     UMF_NOMAGAZINE},
    {"reap_interval", "Private", ITEM_UINT,
     "Minimum time between reaps and updates, in seconds.", NULL, 0,
     &umem_reap_interval},

    {"size_add", "Private", ITEM_SPECIAL, "add a size to the cache size table",
     NULL, 0, NULL, &umem_size_tempval, &umem_size_process},
    {"size_clear", "Private", ITEM_SPECIAL,
     "clear all but the largest size from the cache size table", NULL, 0, NULL,
     &umem_size_tempval, &umem_size_process},
    {"size_remove", "Private", ITEM_SPECIAL,
     "remove a size from the cache size table", NULL, 0, NULL,
     &umem_size_tempval, &umem_size_process},
#ifndef _WIN32
#ifndef UMEM_STANDALONE
    {"sbrk_minalloc", "Private", ITEM_SIZE,
     "The minimum allocation chunk for the sbrk(2) heap.", NULL, 0, NULL,
     &vmem_sbrk_minalloc},
    {"sbrk_pagesize", "Private", ITEM_SIZE,
     "The preferred page size for the sbrk(2) heap.", NULL, 0, NULL,
     &vmem_sbrk_pagesize},
#endif
#endif
...
static umem_env_item_t umem_debug_items[] = {
    {"default", "Unstable", ITEM_FLAG, "audit,contents,guards", &umem_flags,
    ...
    {"guards", "Unstable", ITEM_FLAG, "Enables guards and special patterns",
     &umem_flags, UMF_DEADBEEF | UMF_REDZONE},
    {"verbose", "Unstable", ITEM_FLAG,
     "Enables writing error messages to stderr", &umem_output, 1},
    ...
    {"lite", "Private", ITEM_FLAG, "debugging-lite", &umem_flags, UMF_LITE},
    ...
    {"mtbf", "Private", ITEM_UINT,
     ...
     "if prime.\n",
     NULL, 0, &umem_mtbf},
    {"random", "Private", ITEM_FLAG, "randomize flags on a per-cache basis",
     &umem_flags, UMF_RANDOMIZE},
    {"allverbose", "Private", ITEM_FLAG,
     "Enables writing all logged messages to stderr", &umem_output, 2},
...
static umem_env_item_t umem_logging_items[] = {
    {"transaction", "Unstable", ITEM_SPECIAL,
     ...
     NULL, 0, NULL, &umem_transaction_log_size, &umem_log_process},
    {"contents", "Unstable", ITEM_SPECIAL,
     ...
     &umem_flags, UMF_CONTENTS, NULL, &umem_content_log_size,
     &umem_log_process},
    {"fail", "Unstable", ITEM_SPECIAL,
     "Records are entered into this log for every failed "
     "allocation.",
     NULL, 0, NULL, &umem_failure_log_size, &umem_log_process},

    {"slab", "Private", ITEM_SPECIAL,
     "Every slab created will be entered into this log.", NULL, 0, NULL,
...
static int item_uint_process(const umem_env_item_t *item,
                             const char *item_arg) {
  ulong_t result;
  char *endptr = "";
  int olderrno;
...
static int item_size_process(const umem_env_item_t *item,
                             const char *item_arg) {
  ulong_t result;
  ulong_t result_arg;
  char *endptr = "";
...
static int umem_log_process(const umem_env_item_t *item, const char *item_arg) {
  if (item_arg != NULL) {
    int ret;
...
static int umem_size_process(const umem_env_item_t *item,
                             const char *item_arg) {
...
#ifndef UMEM_STANDALONE
static int umem_backend_process(const umem_env_item_t *item,
                                const char *item_arg) {
  const char *name = item->item_name;
...
static int umem_allocator_process(const umem_env_item_t *item,
                                  const char *item_arg) {
  const char *name = item->item_name;
...
static int process_item(const umem_env_item_t *item, const char *item_arg) {
  int arg_required = 0;
...
void umem_process_value(umem_env_item_t *item_list, const char *beg,
                        const char *end) {
...
#define dlerror() 0
  ...
  if ((h = dlopen(0, RTLD_FIRST | RTLD_LAZY)) != NULL) {
    for (cur_env = umem_envvars; cur_env->env_name != NULL; cur_env++) {
      ...
      state = STATE_DLSYM;
      func = (const char *(*)(void))dlsym(h, cur_env->env_func);
