...
void *malloc(size_t size_arg) {
  ...
  if (ret == NULL) {
    ...
    ret->malloc_stat = UMEM_MALLOC_ENCODE(MALLOC_OVERSIZE_MAGIC, low_size);
    ret++;
    ...
    ret->malloc_size = low_size;
    ret->malloc_stat = UMEM_MALLOC_ENCODE(MALLOC_SECOND_MAGIC, low_size);
    ret++;
#endif
  } else {
...
void *memalign(size_t align, size_t size_arg) {
  ...
  buf = vmem_xalloc(umem_memalign_arena, size, align, phase, 0, NULL, NULL,
                    VM_NOSLEEP);
  ...
  ret = (malloc_data_t *)buf;
  {
    ...
    ret->malloc_size = high_size;
    ret->malloc_stat = UMEM_MALLOC_ENCODE(MEMALIGN_MAGIC, high_size);
    ret++;
#endif
...
void *valloc(size_t size) { return (memalign(pagesize, size)); }
             ...
             size_t *data_size_arg) /* output: bytes of data in buf_arg */
{
  ...
  case MALLOC_OVERSIZE_MAGIC: {
    ...
    if (UMEM_MALLOC_DECODE(buf->malloc_stat, high_size) != MALLOC_MAGIC) {
  ...
  case MEMALIGN_MAGIC: {
    ...
    if (UMEM_MALLOC_DECODE(buf->malloc_stat, high_size) != MEMALIGN_MAGIC) {
      message = "invalid or corrupted buffer";
      break;
    }
    size += high_size << 32;
