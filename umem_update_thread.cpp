...
/*ARGSUSED*/
static THR_RETURN THR_API umem_update_thread(void *arg) {
  struct timeval now;
  int in_update = 0;
  struct umem_suspend_signal_object *obj =
      (struct umem_suspend_signal_object *)arg;
  ...
  for (;;) {
    ...
    if (umem_null_cache.cache_unext == &umem_null_cache) {
      ...
      (void)cond_timedwait(&umem_update_cv, &umem_update_lock, &abs_time);
