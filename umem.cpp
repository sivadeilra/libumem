 ...
 * &umem_null_cache to fail by returning (nullptr), regardless of the
 * flags argument.
 *
 * So umem_cache_alloc() returns nullptr, and umem_alloc()/umem_zalloc() call
 ...
 *
 * Inactive             cache_uflags is zero, cache_u{next,prev} are nullptr
 *
 ...
 *                      are nullptr, and either umem_update_thr or
 ...
 * more memory) instead of failing (returning nullptr).
 *
 ...
 *
 *      * UMEM_DEFAULT, equivalent to KM_NOSLEEP (i.e. return nullptr on
 *      failure)
 *
...
inline uint32_t UMEM_VMFLAGS(uint32_t umflag) { return VM_NOSLEEP; }
...
static int umem_alloc_sizes[] = {
    ...
    6 * 128,
    7 * 128,
    (int)P2ALIGN(8192 / 7, 64),
    (int)P2ALIGN(8192 / 6, 64),
    (int)P2ALIGN(8192 / 5, 64),
    (int)P2ALIGN(8192 / 4, 64),
    2304,
    (int)P2ALIGN(8192 / 3, 64),
    (int)P2ALIGN(8192 / 2, 64),
    4544,
    (int)P2ALIGN(8192 / 1, 64),
    9216,
    4096 * 3,
    UMEM_MAXBUF, /* = 8192 * 2 */
    24576,
...
constexpr size_t NUM_ALLOC_SIZES =
    (sizeof(umem_alloc_sizes) / sizeof(*umem_alloc_sizes));
...
uint32_t umem_depot_contention = 2; /* max failed trylocks per real interval */
uint32_t umem_abort = 1;            /* whether to abort on error */
uint32_t umem_output = 0;           /* whether to write to standard error */
uint32_t umem_logging = 0;          /* umem_log_enter() override */
...
bool IN_UPDATE() {
  return (thr_self() == umem_update_thr || thr_self() == umem_st_update_thr);
}
bool IN_REAP() { return IN_UPDATE(); }
...
umem_cache_t umem_null_cache = {
    ...
    {&umem_null_cache, nullptr, &umem_null_cache.cache_nullslab,
     &umem_null_cache.cache_nullslab, nullptr, -1, 0},
    nullptr,
    nullptr,
...
static umem_cache_t *umem_alloc_table[UMEM_MAXBUF >> UMEM_ALIGN_SHIFT] = {
    ALLOC_TABLE_1024, ALLOC_TABLE_1024, ALLOC_TABLE_1024, ALLOC_TABLE_1024,
    ALLOC_TABLE_1024, ALLOC_TABLE_1024, ALLOC_TABLE_1024, ALLOC_TABLE_1024,
    ALLOC_TABLE_1024, ALLOC_TABLE_1024, ALLOC_TABLE_1024, ALLOC_TABLE_1024,
    ALLOC_TABLE_1024, ALLOC_TABLE_1024, ALLOC_TABLE_1024, ALLOC_TABLE_1024};
...
constexpr UMERR UMERR_MODIFIED = 0; /* buffer modified while on freelist */
constexpr UMERR UMERR_REDZONE =
    1; /* redzone violation (write past end of buf) */
constexpr UMERR UMERR_DUPFREE = 2;   /* freed a buffer twice */
constexpr UMERR UMERR_BADADDR = 3;   /* freed a bad (unallocated) address */
constexpr UMERR UMERR_BADBUFTAG = 4; /* buftag corrupted */
constexpr UMERR UMERR_BADBUFCTL = 5; /* bufctl corrupted */
constexpr UMERR UMERR_BADCACHE = 6;  /* freed a buffer to the wrong cache */
constexpr UMERR UMERR_BADSIZE = 7;   /* alloc size != free size */
constexpr UMERR UMERR_BADBASE = 8;   /* buffer base address wrong */

struct umem_abort_info_ {
...
static void *verify_pattern(uint64_t pattern, void *buf_arg, size_t size) {
  uint64_t *bufend = (uint64_t *)((char *)buf_arg + size);
  uint64_t *buf;

  for (buf = (uint64_t *)buf_arg; buf < bufend; buf++) {
    if (*buf != pattern) {
      return (buf);
    }
  }
...
static void *verify_and_copy_pattern(uint64_t old, uint64_t _new, void *buf_arg,
                                     size_t size) {
  uint64_t *bufend = (uint64_t *)((char *)buf_arg + size);
  uint64_t *buf;
  ...
  for (buf = (uint64_t *)buf_arg; buf < bufend; buf++) {
    if (*buf != old) {
      copy_pattern(old, buf_arg, (char *)buf - (char *)buf_arg);
      return (buf);
    }
    *buf = _new;
  }
...
static void umem_add_update_unlocked(umem_cache_t *cp, int flags) {
...
static void umem_add_update(umem_cache_t *cp, int flags) {
 ...
 * complete first.
 */
static void umem_remove_updates(umem_cache_t *cp) {
  (void)mutex_lock(&umem_update_lock);
  ...
  while (cp->cache_uflags & UMU_ACTIVE) {
    ...
    ASSERT(umem_update_thr != thr_self() && umem_st_update_thr != thr_self());

    (void)pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &cancel_state);
    (void)cond_wait(&umem_update_cv, &umem_update_lock);
    (void)pthread_setcancelstate(cancel_state, nullptr);
  }
  /*
  ...
  if (cp->cache_unext != nullptr) {
    ...
    cp->cache_uprev = cp->cache_unext = nullptr;
    cp->cache_uflags = 0;
  }
  /*
   * Make sure it is in the Inactive state
   */
  ASSERT(cp->cache_unext == nullptr && cp->cache_uflags == 0);
  (void)mutex_unlock(&umem_update_lock);
}

static void umem_updateall(int flags) {
  umem_cache_t *cp;
 ...
 */
static umem_slab_t *umem_findslab(umem_cache_t *cp, void *buf) {
  umem_slab_t *sp;
...
static void umem_error(int error, umem_cache_t *cparg, void *bufarg) {
  umem_buftag_t *btp = nullptr;
  umem_bufctl_t *bcp = nullptr;
  ...
  if (sp == nullptr) {
    for (cp = umem_null_cache.cache_prev; cp != &umem_null_cache;
         cp = cp->cache_prev) {
      if ((sp = umem_findslab(cp, buf)) != nullptr) {
        break;
      }
    }
  }
  ...
  if (sp == nullptr) {
    ...
    if (cp != cparg) {
      ...
      buf = (char *)bufarg - ((uintptr_t)bufarg - (uintptr_t)sp->slab_base) %
                                 cp->cache_chunksize;
    }
    if (buf != bufarg) {
      error = UMERR_BADBASE;
    ...
    if (cp->cache_flags & UMF_HASH) {
      ...
      (void)mutex_unlock(&cp->cache_lock);
      if (bcp == nullptr && btp != nullptr) {
        bcp = btp->bt_bufctl;
      }
      if (umem_findslab(cp->cache_bufctl_cache, bcp) == nullptr ||
          P2PHASE((uintptr_t)bcp, UMEM_ALIGN) || bcp->bc_addr != buf) {
  ...
  case UMERR_MODIFIED:
    ...
    off = (uint64_t *)verify_pattern(UMEM_FREE_PATTERN, buf, cp->cache_verify);
    if (off == nullptr) { /* shouldn't happen */
      off = (uint64_t *)buf;
    }
    umem_printf("modification occurred at offset 0x%lx "
                "(0x%llx replaced by 0x%llx)\n",
                (uintptr_t)off - (uintptr_t)buf, (longlong_t)UMEM_FREE_PATTERN,
                (longlong_t)*off);
    break;
  ...
  case UMERR_BADBUFTAG:
    ...
    umem_printf("bcp ^ bxstat = %lx, should be %lx\n",
                (intptr_t)btp->bt_bufctl ^ btp->bt_bxstat, UMEM_BUFTAG_FREE);
    break;
  ...
  if (bcp != nullptr && (cp->cache_flags & UMF_AUDIT) &&
      error != UMERR_BADBUFCTL) {
    int d;
    timespec_t ts;
    hrtime_t diff;
    ...
    for (d = 0; d < std::min<size_t>(bcap->bc_depth, umem_stack_depth); d++) {
...
void umem_nofail_callback(umem_nofail_callback_t *cb) { nofail_callback = cb; }

static int umem_alloc_retry(umem_cache_t *cp, int umflag) {
...
static umem_log_header_t *umem_log_init(size_t logsize) {
  ...
  lhp = (umem_log_header_t *)vmem_xalloc(umem_log_arena, lhsize, 64,
                                         P2NPHASE(lhsize, 64), 0, nullptr, nullptr,
                                         VM_NOSLEEP);
  if (lhp == nullptr) {
    goto fail;
  }
  ...
  (void)mutex_init(&lhp->lh_lock, USYNC_THREAD, nullptr);
  ...
  lhp->lh_base = (char *)vmem_alloc(umem_log_arena, lhp->lh_chunksize * nchunks,
                                    VM_NOSLEEP);
  if (lhp->lh_base == nullptr) {
    goto fail;
  }

  lhp->lh_free =
      (int *)vmem_alloc(umem_log_arena, nchunks * sizeof(int), VM_NOSLEEP);
  if (lhp->lh_free == nullptr) {
    goto fail;
  }
  ...
  for (i = 0; i < umem_max_ncpus; i++) {
    ...
    (void)mutex_init(&clhp->clh_lock, USYNC_THREAD, nullptr);
    clhp->clh_chunk = i;
  }
...
fail:
  if (lhp != nullptr) {
    if (lhp->lh_base != nullptr) {
      vmem_free(umem_log_arena, lhp->lh_base, lhp->lh_chunksize * nchunks);
    }
...
static void *umem_log_enter(umem_log_header_t *lhp, void *data, size_t size) {
  void *logspace;
  umem_cpu_log_header_t *clhp = &lhp->lh_cpu[CPU(umem_cpu_mask)->cpu_number];

  if (lhp == nullptr || umem_logging == 0) {
    return (nullptr);
  }
  ...
  if (size > clhp->clh_avail) {
    ...
    clhp->clh_current = lhp->lh_base + clhp->clh_chunk * lhp->lh_chunksize;
...
void UMEM_AUDIT(umem_log_header_t *lp, umem_cache_t *cp,
                umem_bufctl_audit_t *bcp) {
  umem_bufctl_audit_t *_bcp = bcp;
  _bcp->bc_timestamp = gethrtime();
  _bcp->bc_thread = thr_self();
  _bcp->bc_depth =
      getpcstack(_bcp->bc_stack, umem_stack_depth,
                 (cp != nullptr) && (cp->cache_flags & UMF_CHECKSIGNAL));
  _bcp->bc_lastlog =
      (umem_bufctl *)umem_log_enter((lp), _bcp, UMEM_BUFCTL_AUDIT_SIZE);
}

static void umem_log_event(umem_log_header_t *lp, umem_cache_t *cp,
 ...
 */
static umem_slab_t *umem_slab_create(umem_cache_t *cp, int umflag) {
  ...
  slab = (char *)vmem_alloc(vmp, slabsize, UMEM_VMFLAGS(umflag));
  ...
  if (!(cp->cache_cflags & UMC_NOTOUCH) && (cp->cache_flags & UMF_DEADBEEF)) {
    copy_pattern(UMEM_UNINITIALIZED_PATTERN, slab, slabsize);
  }

  if (cache_flags & UMF_HASH) {
    if ((sp = (umem_slab_t *)_umem_cache_alloc(umem_slab_cache, umflag)) ==
        nullptr) {
  ...
  while (chunks-- != 0) {
    if (cache_flags & UMF_HASH) {
      bcp = (umem_bufctl_t *)_umem_cache_alloc(cp->cache_bufctl_cache, umflag);
      if (bcp == nullptr) {
...
bufctl_alloc_failure:
  ...
  while ((bcp = sp->slab_head) != nullptr) {
...
vmem_alloc_failure:
  ...
  umem_log_event(umem_failure_log, cp, nullptr, nullptr);
 ...
 * Destroy a slab.
 */
static void umem_slab_destroy(umem_cache_t *cp, umem_slab_t *sp) {
  vmem_t *vmp = cp->cache_arena;
 ...
 */
static void *umem_slab_alloc(umem_cache_t *cp, int umflag) {
  ...
  if (sp->slab_head == nullptr) {
    ...
    if ((sp = umem_slab_create(cp, umflag)) == nullptr) {
      return (nullptr);
    }
    (void)mutex_lock(&cp->cache_lock);
   ...
   */
  bcp = sp->slab_head;
  if ((sp->slab_head = bcp->bc_next) == nullptr) {
  ...
  if (cp->cache_flags & UMF_HASH) {
    ...
    *hash_bucket = bcp;
    if ((cp->cache_flags & (UMF_AUDIT | UMF_BUFTAG)) == UMF_AUDIT) {
      UMEM_AUDIT((umem_log_header_t *)umem_transaction_log, cp,
                 (umem_bufctl_audit_t *)bcp);
    }
  } else {
    buf = UMEM_BUF(cp, bcp);
 ...
 */
static void umem_slab_free(umem_cache_t *cp, void *buf) {
  umem_slab_t *sp;
  ...
  if (bcp == nullptr || sp->slab_cache != cp || !UMEM_SLAB_MEMBER(sp, buf)) {
    (void)mutex_unlock(&cp->cache_lock);
  ...
  if ((cp->cache_flags & (UMF_AUDIT | UMF_BUFTAG)) == UMF_AUDIT) {
    if (cp->cache_flags & UMF_CONTENTS) {
      ((umem_bufctl_audit_t *)bcp)->bc_contents =
          umem_log_enter(umem_content_log, buf, cp->cache_contents);
    }
    UMEM_AUDIT(umem_transaction_log, cp, (umem_bufctl_audit_t *)bcp);
  }
...
static int umem_cache_alloc_debug(umem_cache_t *cp, void *buf, int umflag) {
  umem_buftag_t *btp = UMEM_BUFTAG(cp, buf);
  ...
  if (cp->cache_flags & UMF_DEADBEEF) {
    if (verify_and_copy_pattern(UMEM_FREE_PATTERN, UMEM_UNINITIALIZED_PATTERN,
                                buf, cp->cache_verify)) {
  ...
  if ((mtbf = umem_mtbf | cp->cache_mtbf) != 0 && gethrtime() % mtbf == 0 &&
...
static int umem_cache_free_debug(umem_cache_t *cp, void *buf) {
  umem_buftag_t *btp = UMEM_BUFTAG(cp, buf);
  umem_bufctl_audit_t *bcp = (umem_bufctl_audit_t *)btp->bt_bufctl;
  umem_slab_t *sp;
  ...
  if (btp->bt_bxstat != ((intptr_t)bcp ^ UMEM_BUFTAG_ALLOC)) {
    ...
    sp = umem_findslab(cp, buf);
    if (sp == nullptr || sp->slab_cache != cp) {
      umem_error(UMERR_BADADDR, cp, buf);
    } else {
  ...
  if (cp->cache_destructor != nullptr) {
 ...
 */
static void umem_magazine_destroy(umem_cache_t *cp, umem_magazine_t *mp,
                                  int nrounds) {
  int round;

  ASSERT(cp->cache_next == nullptr || IN_UPDATE());

  for (round = 0; round < nrounds; round++) {
    void *buf = mp->mag_round[round];

    if ((cp->cache_flags & UMF_DEADBEEF) &&
        verify_pattern(UMEM_FREE_PATTERN, buf, cp->cache_verify) != nullptr) {
      umem_error(UMERR_MODIFIED, cp, buf);
      continue;
    }

    if (!(cp->cache_flags & UMF_BUFTAG) && cp->cache_destructor != nullptr) {
      cp->cache_destructor(buf, cp->cache_private);
    }
 ...
 */
static umem_magazine_t *umem_depot_alloc(umem_cache_t *cp,
                                         umem_maglist_t *mlp) {
  umem_magazine_t *mp;
  ...
  if ((mp = mlp->ml_list) != nullptr) {
    ASSERT(UMEM_MAGAZINE_VALID(cp, mp));
    mlp->ml_list = (umem_magazine_t *)mp->mag_next;
 ...
 */
static void umem_depot_free(umem_cache_t *cp, umem_maglist_t *mlp,
                            umem_magazine_t *mp) {
  (void)mutex_lock(&cp->cache_depot_lock);
  ASSERT(UMEM_MAGAZINE_VALID(cp, mp));
 ...
 */
static void umem_depot_ws_update(umem_cache_t *cp) {
  (void)mutex_lock(&cp->cache_depot_lock);
 ...
 */
static void umem_depot_ws_reap(umem_cache_t *cp) {
  long reap;
  umem_magazine_t *mp;

  ASSERT(cp->cache_next == nullptr || IN_REAP());

  reap = std::min<long>(cp->cache_full.ml_reaplimit, cp->cache_full.ml_min);
  while (reap-- && (mp = umem_depot_alloc(cp, &cp->cache_full)) != nullptr) {
    umem_magazine_destroy(cp, mp, cp->cache_magtype->mt_magsize);
  }

  reap = std::min<long>(cp->cache_empty.ml_reaplimit, cp->cache_empty.ml_min);
  while (reap-- && (mp = umem_depot_alloc(cp, &cp->cache_empty)) != nullptr) {
    umem_magazine_destroy(cp, mp, 0);
  }
}

static void umem_cpu_reload(umem_cpu_cache_t *ccp, umem_magazine_t *mp,
                            int rounds) {
  ASSERT((ccp->cc_loaded == nullptr && ccp->cc_rounds == -1) ||
...
retry:
   ...
   */
  flags_nfatal = UMEM_DEFAULT;
  if (cp->cache_constructor != nullptr &&
      cp->cache_constructor(buf, cp->cache_private, flags_nfatal) != 0) {
    atomic_add_64(&cp->cache_alloc_fail, 1);
    umem_slab_free(cp, buf);
...
void _umem_cache_free(umem_cache_t *cp, void *buf) {
  ...
  for (;;) {
    ...
    if (emp != nullptr) {
      if (ccp->cc_ploaded != nullptr) {
        umem_depot_free(cp, &cp->cache_full, ccp->cc_ploaded);
      }
    ...
    (void)mutex_unlock(&ccp->cc_lock);
    emp = (umem_magazine_t *)_umem_cache_alloc(mtp->mt_cache, UMEM_DEFAULT);
    (void)mutex_lock(&ccp->cc_lock);
...
umem_alloc_retry:
  if (index<UMEM_MAXBUF>> UMEM_ALIGN_SHIFT) {
    umem_cache_t *cp = umem_alloc_table[index];
    buf = _umem_cache_alloc(cp, umflag);
    if ((cp->cache_flags & UMF_BUFTAG) && buf != nullptr) {
      umem_buftag_t *btp = UMEM_BUFTAG(cp, buf);
      ((uint8_t *)buf)[size] = UMEM_REDZONE_BYTE;
      ((uint32_t *)btp)[1] = UMEM_SIZE_ENCODE(size);
    }
    if (buf == nullptr && umem_alloc_retry(cp, umflag)) {
  ...
  if (umem_oversize_arena == nullptr) {
    if (umem_init()) {
      ASSERT(umem_oversize_arena != nullptr);
    } else {
      return (nullptr);
    }
  }
  ...
  if (buf == nullptr) {
    umem_log_event(umem_failure_log, nullptr, nullptr, (void *)size);
    if (umem_alloc_retry(nullptr, umflag)) {
      goto umem_alloc_retry;
    }
  }
  return (buf);
...
umem_alloc_align_retry:
  if (umem_memalign_arena == nullptr) {
    if (umem_init()) {
      ASSERT(umem_oversize_arena != nullptr);
    } else {
      return (nullptr);
  ...
  buf = vmem_xalloc(umem_memalign_arena, size, align, 0, 0, nullptr, nullptr,
                    UMEM_VMFLAGS(umflag));
  if (buf == nullptr) {
    umem_log_event(umem_failure_log, nullptr, nullptr, (void *)size);
    if (umem_alloc_retry(nullptr, umflag)) {
...
static void *umem_firewall_va_alloc(vmem_t *vmp, size_t size, int vmflag) {
...
static void umem_firewall_va_free(vmem_t *vmp, void *addr, size_t size) {
 ...
 */
static void umem_cache_reap(umem_cache_t *cp) {
  /*
 ...
 */
static void umem_cache_magazine_purge(umem_cache_t *cp) {
  umem_cpu_cache_t *ccp;
  ...
  for (cpu_seqid = 0; cpu_seqid < umem_max_ncpus; cpu_seqid++) {
    ...
    ccp->cc_loaded = nullptr;
    ccp->cc_ploaded = nullptr;
    ccp->cc_rounds = -1;
    ccp->cc_prounds = -1;
    ccp->cc_magsize = 0;
    (void)mutex_unlock(&ccp->cc_lock);
 ...
 */
static void umem_cache_magazine_enable(umem_cache_t *cp) {
  int cpu_seqid;
 ...
 */
static void umem_cache_magazine_resize(umem_cache_t *cp) {
  ...
  if (cp->cache_chunksize < mtp->mt_maxbuf) {
    ...
    cp->cache_depot_contention_prev = cp->cache_depot_contention + INT_MAX;
    (void)mutex_unlock(&cp->cache_depot_lock);
 ...
 */
static void umem_hash_rescale(umem_cache_t *cp) {
  umem_bufctl_t **old_table, **new_table, *bcp;
  size_t old_size, new_size, h;

  ASSERT(IN_UPDATE());

  new_size = std::max<size_t>(UMEM_HASH_INITIAL,
  ...
  new_table = (umem_bufctl_t **)vmem_alloc(
      umem_hash_arena, new_size * sizeof(void *), VM_NOSLEEP);
...
void umem_cache_update(umem_cache_t *cp) {
  ...
  if (cp->cache_chunksize < cp->cache_magtype->mt_maxbuf &&
      (int)(cp->cache_depot_contention - cp->cache_depot_contention_prev) >
...
#ifndef UMEM_STANDALONE
static void umem_st_update(void) {
  ASSERT(MUTEX_HELD(&umem_update_lock));
  ASSERT(umem_update_thr == 0 && umem_st_update_thr == 0);
  ...
  umem_reap_next = gethrtime() + (hrtime_t)umem_reap_interval * NANOSEC;
...
umem_cache_t *
umem_cache_create(char *name,     /* descriptive name for this cache */
                  ...
                  void *_private, /* pass-thru arg for constr/destr/reclaim */
                  ...
                  int cflags)     /* cache creation flags */
{
  ...
  if ((align & (align - 1)) != 0 || align > vmp->vm_quantum ||
      ((cflags & UMC_NOHASH) && (cflags & UMC_NOTOUCH)) || name == nullptr ||
      bufsize == 0) {
    errno = EINVAL;
    return (nullptr);
  }
   ...
   */
  cp = (umem_cache_t *)vmem_xalloc(umem_cache_arena, csize, UMEM_CPU_CACHE_SIZE,
                                   phase, 0, nullptr, nullptr, VM_NOSLEEP);
  ...
  if (umem_flags & UMF_RANDOMIZE) {
    umem_flags =
        (((umem_flags | ~UMF_RANDOM) + 1) & UMF_RANDOM) | UMF_RANDOMIZE;
  }
  ...
  if (cp->cache_flags & UMF_LITE) {
    if (bufsize >= umem_lite_minsize && align <= umem_lite_maxalign &&
  ...
  if (vmp != umem_default_arena || umem_firewall_arena == nullptr) {
    cp->cache_flags &= ~UMF_FIREWALL;
  }
  ...
  if (cp->cache_flags & UMF_DEADBEEF) {
    cp->cache_verify = std::min<size_t>(cp->cache_buftag, umem_maxverify);
    if (cp->cache_flags & UMF_LITE) {
      cp->cache_verify = std::min<size_t>(cp->cache_verify, UMEM_ALIGN);
    }
  }

  cp->cache_contents = std::min<size_t>(cp->cache_bufctl, umem_content_maxsave);
  ...
  if (vmp == umem_firewall_arena) {
  ...
  } else if ((cflags & UMC_NOHASH) ||
             (!(cflags & UMC_NOTOUCH) && !(cp->cache_flags & UMF_AUDIT) &&
              chunksize < vmp->vm_quantum / UMEM_VOID_FRACTION)) {
    cp->cache_slabsize = vmp->vm_quantum;
    cp->cache_mincolor = 0;
    cp->cache_maxcolor = (cp->cache_slabsize - sizeof(umem_slab_t)) % chunksize;
    ...
    for (chunks = 1; chunks <= UMEM_VOID_FRACTION; chunks++) {
      slabsize = P2ROUNDUP(chunksize * chunks, vmp->vm_quantum);
      /*
       * check for overflow
       */
    ...
    if (cflags & UMC_QCACHE) {
      bestfit = std::max<size_t>(1 << highbit(3 * vmp->vm_qcache_max), 64);
    }
   ...
   */
  (void)mutex_init(&cp->cache_lock, USYNC_THREAD, nullptr);
  ...
  if (cp->cache_flags & UMF_HASH) {
    cp->cache_hash_table = (umem_bufctl_t **)vmem_alloc(
        umem_hash_arena, UMEM_HASH_INITIAL * sizeof(void *), VM_NOSLEEP);
    if (cp->cache_hash_table == nullptr) {
      errno = EAGAIN;
      goto fail_lock;
    }
    bzero(cp->cache_hash_table, UMEM_HASH_INITIAL * sizeof(void *));
   ...
   */
  (void)mutex_init(&cp->cache_depot_lock, USYNC_THREAD, nullptr);
  ...
  for (cpu_seqid = 0; cpu_seqid < umem_max_ncpus; cpu_seqid++) {
    umem_cpu_cache_t *ccp = &cp->cache_cpu[cpu_seqid];
    (void)mutex_init(&ccp->cc_lock, USYNC_THREAD, nullptr);
...
void umem_cache_destroy(umem_cache_t *cp) {
  ...
  cp->cache_prev = cp->cache_next = nullptr;
  (void)mutex_unlock(&umem_cache_lock);
...
void umem_alloc_sizes_clear(void) {
  int i;
...
void umem_alloc_sizes_add(size_t size_arg) {
  int i, j;
...
void umem_alloc_sizes_remove(size_t size) {
  int i;
...
static int umem_cache_init(void) {
  int i;
  ...
  for (i = 0; i < sizeof(umem_magtype) / sizeof(*mtp); i++) {
    mtp = &umem_magtype[i];
    (void)snprintf(name, sizeof(name), "umem_magazine_%d", mtp->mt_magsize);
    mtp->mt_cache = umem_cache_create(
        name, (mtp->mt_magsize + 1) * sizeof(void *), mtp->mt_align, nullptr, nullptr,
        nullptr, nullptr, umem_internal_arena, UMC_NOHASH | UMC_INTERNAL);
    if (mtp->mt_cache == nullptr) {
      return (0);
    }
  }

  umem_slab_cache = umem_cache_create(
      "umem_slab_cache", sizeof(umem_slab_t), 0, nullptr, nullptr, nullptr, nullptr,
      umem_internal_arena, UMC_NOHASH | UMC_INTERNAL);

  if (umem_slab_cache == nullptr) {
    return (0);
  }

  umem_bufctl_cache = umem_cache_create(
      "umem_bufctl_cache", sizeof(umem_bufctl_t), 0, nullptr, nullptr, nullptr, nullptr,
      umem_internal_arena, UMC_NOHASH | UMC_INTERNAL);

  if (umem_bufctl_cache == nullptr) {
    return (0);
  }
  ...
  umem_bufctl_audit_cache =
      umem_cache_create("umem_bufctl_audit_cache", size, 0, nullptr, nullptr, nullptr,
                        nullptr, umem_internal_arena, UMC_NOHASH | UMC_INTERNAL);

  if (umem_bufctl_audit_cache == nullptr) {
    return (0);
  }

  if (vmem_backend & VMEM_BACKEND_MMAP) {
    umem_va_arena =
        vmem_create("umem_va", 0, 0, pagesize, vmem_alloc, vmem_free,
                    heap_arena, 8 * pagesize, VM_NOSLEEP);
  } else {
    umem_va_arena = heap_arena;
  }

  if (umem_va_arena == nullptr) {
    return (0);
  }

  umem_default_arena =
      vmem_create("umem_default", 0, 0, pagesize, heap_alloc, heap_free,
                  umem_va_arena, 0, VM_NOSLEEP);

  if (umem_default_arena == nullptr) {
    return (0);
  }
  ...
  for (i = 0; i < NUM_ALLOC_SIZES; i++) {
    ...
    (void)snprintf(name, sizeof(name), "umem_alloc_%lu", (long)cache_size);

    cp = umem_cache_create(name, cache_size, align, nullptr, nullptr, nullptr, nullptr,
                           nullptr, UMC_INTERNAL);
    if (cp == nullptr) {
      return (0);
    }
 ...
 */
void umem_startup(caddr_t start, size_t len, size_t pagesize, caddr_t minstack,
                  caddr_t maxstack) {
#ifdef UMEM_STANDALONE
  int idx;
  ...
  nofail_callback = nullptr;
  umem_slab_cache = nullptr;
  umem_bufctl_cache = nullptr;
  umem_bufctl_audit_cache = nullptr;
  heap_arena = nullptr;
  heap_alloc = nullptr;
  heap_free = nullptr;
  umem_internal_arena = nullptr;
  umem_cache_arena = nullptr;
  umem_hash_arena = nullptr;
  umem_log_arena = nullptr;
  umem_oversize_arena = nullptr;
  umem_va_arena = nullptr;
  umem_default_arena = nullptr;
  umem_firewall_va_arena = nullptr;
  umem_firewall_arena = nullptr;
  umem_memalign_arena = nullptr;
  umem_transaction_log = nullptr;
  umem_content_log = nullptr;
  umem_failure_log = nullptr;
  umem_slab_log = nullptr;
  umem_cpu_mask = 0;
  ...
  bcopy(&umem_null_cache_template, &umem_null_cache, sizeof(umem_cache_t));
...
int umem_init(void) {
  ...
  if (thr_self() != umem_init_thr) {
    ...
    if (umem_ready != UMEM_READY_STARTUP) {
      ...
      while (umem_ready == UMEM_READY_INITING) {
        ...
        (void)pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &cancel_state);
        (void)cond_wait(&umem_init_cv, &umem_init_lock);
        (void)pthread_setcancelstate(cancel_state, nullptr);
      }
      ASSERT(umem_ready == UMEM_READY || umem_ready == UMEM_READY_INIT_FAILED);
    ...
    umem_panic("recursive allocation while initializing umem\n");
  }
  ...
  umem_internal_arena =
      vmem_create("umem_internal", 0, 0, pagesize, heap_alloc, heap_free,
                  heap_arena, 0, VM_NOSLEEP);

  umem_default_arena = umem_internal_arena;

  if (umem_internal_arena == nullptr) {
    goto fail;
  }

  umem_cache_arena = vmem_create("umem_cache", 0, 0, UMEM_ALIGN, vmem_alloc,
                                 vmem_free, umem_internal_arena, 0, VM_NOSLEEP);

  umem_hash_arena = vmem_create("umem_hash", 0, 0, UMEM_ALIGN, vmem_alloc,
                                vmem_free, umem_internal_arena, 0, VM_NOSLEEP);

  umem_log_arena = vmem_create("umem_log", 0, 0, UMEM_ALIGN, heap_alloc,
                               heap_free, heap_arena, 0, VM_NOSLEEP);

  umem_firewall_va_arena =
      vmem_create("umem_firewall_va", 0, 0, pagesize, umem_firewall_va_alloc,
                  umem_firewall_va_free, heap_arena, 0, VM_NOSLEEP);

  if (umem_cache_arena == nullptr || umem_hash_arena == nullptr ||
      umem_log_arena == nullptr || umem_firewall_va_arena == nullptr) {
    goto fail;
  }

  umem_firewall_arena =
      vmem_create("umem_firewall", 0, 0, pagesize, heap_alloc, heap_free,
                  umem_firewall_va_arena, 0, VM_NOSLEEP);

  if (umem_firewall_arena == nullptr) {
    goto fail;
  }

  oversize_arena =
      vmem_create("umem_oversize", 0, 0, pagesize, heap_alloc, heap_free,
                  minfirewall < ULONG_MAX ? umem_firewall_va_arena : heap_arena,
                  0, VM_NOSLEEP);

  memalign_arena =
      vmem_create("umem_memalign", 0, 0, UMEM_ALIGN, heap_alloc, heap_free,
                  minfirewall < ULONG_MAX ? umem_firewall_va_arena : heap_arena,
                  0, VM_NOSLEEP);

  if (oversize_arena == nullptr || memalign_arena == nullptr) {
    goto fail;
  }
  ...
  new_cpus = (umem_cpu_t *)vmem_alloc(umem_internal_arena, size, VM_NOSLEEP);
  if (new_cpus == nullptr) {
    goto fail;
  }
...
#ifndef UMEM_STANDALONE
  (void)gettimeofday(&umem_update_next, nullptr);
#endif
...
#ifndef UMEM_STANDALONE
void __attribute__((constructor)) __umem_init(void) {
  umem_startup(nullptr, 0, 0, nullptr, nullptr);
}
#endif
