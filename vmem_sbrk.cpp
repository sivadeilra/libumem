...
static sbrk_fail_t sbrk_fails = {&sbrk_fails, &sbrk_fails, NULL, 0};
 ...
 */
static void *_sbrk_grow_aligned(size_t min_size, size_t low_align,
 ...
 */
static void *vmem_sbrk_extend_alloc(vmem_t *src, void *pos, size_t size,
                                    size_t alloc, int vmflags) {
...
static void *vmem_sbrk_tryfail(vmem_t *src, size_t size, int vmflags) {
  ...
  if (fp != &sbrk_fails) {
    ...
    return (vmem_sbrk_extend_alloc(src, fp, fp->sf_size, size, vmflags));
  }
  /*
 ...
 * fork1() support
 */
void vmem_sbrk_lockup(void) { (void)mutex_lock(&sbrk_faillock); }

void vmem_sbrk_release(void) { (void)mutex_unlock(&sbrk_faillock); }
...
vmem_t *vmem_sbrk_arena(vmem_alloc_t **a_out, vmem_free_t **f_out) {
  if (sbrk_heap == NULL) {
    ...
    if (heap_size <= real_pagesize) {
      ...
      if (memcntl(NULL, 0, MC_HAT_ADVISE, (char *)&mha, 0, 0) == -1) {
    ...
    sbrk_heap =
        vmem_init("sbrk_top", real_pagesize, vmem_sbrk_alloc, vmem_free,
                  "sbrk_heap", NULL, 0, real_pagesize, vmem_alloc, vmem_free);
  }
