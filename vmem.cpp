...
constexpr size_t VMEM_INITIAL = 6;       /* early vmem arenas */
constexpr size_t VMEM_SEG_INITIAL = 100; /* early segments */
 ...
 */
constexpr size_t VMEM_SEGS_PER_SPAN_CREATE = 2;
 ...
 */
constexpr size_t VMEM_SEGS_PER_EXACT_ALLOC = 0;
constexpr size_t VMEM_SEGS_PER_LEFT_ALLOC = 1;
constexpr size_t VMEM_SEGS_PER_RIGHT_ALLOC = 1;
constexpr size_t VMEM_SEGS_PER_MIDDLE_ALLOC = 2;
 ...
 */
constexpr size_t VMEM_SEGS_PER_ALLOC_MAX =
 ...
 */
constexpr size_t VMEM_POPULATE_SEGS_PER_ARENA = 4;
constexpr size_t VMEM_POPULATE_LOCKS = 1;

constexpr size_t VMEM_POPULATE_RESERVE =
 ...
 */
constexpr size_t VMEM_MINFREE = VMEM_POPULATE_RESERVE + VMEM_SEGS_PER_ALLOC_MAX;
...
static vmem_populate_lock_t vmem_nosleep_lock = {DEFAULTMUTEX, 0};

inline bool IN_POPULATE() { return vmem_nosleep_lock.vmpl_thr == thr_self(); }
...
inline void VMEM_INSERT_k(vmem_seg_t *vprev, vmem_seg_t *vsp) {
  vmem_seg_t *vnext = vprev->vs_knext;
  vsp->vs_knext = vnext;
  vsp->vs_kprev = vprev;
  vprev->vs_knext = vsp;
  vnext->vs_kprev = vsp;
...
inline void VMEM_INSERT_a(vmem_seg_t *vprev, vmem_seg_t *vsp) {
  vmem_seg_t *vnext = vprev->vs_anext;
  vsp->vs_anext = vnext;
  vsp->vs_aprev = vprev;
  vprev->vs_anext = vsp;
  vnext->vs_aprev = vsp;
...
void VMEM_DELETE_a(vmem_seg_t *vsp) {
  vmem_seg_t *vprev = vsp->vs_aprev;
  vmem_seg_t *vnext = vsp->vs_anext;
  vprev->vs_anext = vnext;
  vnext->vs_aprev = vprev;
}

void VMEM_DELETE_k(vmem_seg_t *vsp) {
  vmem_seg_t *vprev = vsp->vs_kprev;
  vmem_seg_t *vnext = vsp->vs_knext;
  vprev->vs_knext = vnext;
  vnext->vs_kprev = vprev;
}
...
static void vmem_hash_insert(vmem_t *vmp, vmem_seg_t *vsp) {
  ...
  *bucket = vsp;
  ...
  if (vmem_seg_size == sizeof(vmem_seg_t)) {
    vsp->vs_depth = (uint8_t)getpcstack(vsp->vs_stack, VMEM_STACK_DEPTH, 0);
...
static vmem_seg_t *vmem_hash_delete(vmem_t *vmp, uintptr_t addr, size_t size) {
  ...
  if (vsp == nullptr) {
    umem_panic("vmem_hash_delete(%p, %lx, %lu): bad free", vmp, addr, size);
  }
  if (VS_SIZE(vsp) != size) {
 ...
 */
static vmem_seg_t *vmem_seg_create(vmem_t *vmp, vmem_seg_t *vprev,
 ...
 */
static vmem_seg_t *vmem_span_create(vmem_t *vmp, Vaddr vaddr, size_t size,
                                    uint8_t import) {
  vmem_seg_t *knext;
  ...
  knext = &vmp->vm_seg0;
  if (!import && vmp->vm_source_alloc == nullptr) {
     ...
     */
    kend = &vmp->vm_seg0;
    for (kprev = kend->vs_kprev; kprev != kend; kprev = kprev->vs_kprev) {
  ...
  if ((start | end) & (vmp->vm_quantum - 1)) {
    umem_panic("vmem_span_create(%p, %p, %lu): misaligned", vmp, vaddr, size);
  }

  span = vmem_seg_create(vmp, knext->vs_aprev, start, end);
  span->vs_type = VMEM_SPAN;
  VMEM_INSERT_k(knext->vs_kprev, span);
 ...
 */
static void vmem_span_destroy(vmem_t *vmp, vmem_seg_t *vsp) {
 ...
 */
static vmem_seg_t *vmem_seg_alloc(vmem_t *vmp, vmem_seg_t *vsp, uintptr_t addr,
                                  size_t size) {
  ...
  if (vs_end != addr_end) {
    vmem_freelist_insert(vmp, vmem_seg_create(vmp, vsp, addr_end, vs_end));
  }
...
static int vmem_populate(vmem_t *vmp, int vmflag) {
  ...
  while (vmp->vm_nsegfree < VMEM_MINFREE &&
         (vsp = vmem_getseg_global()) != nullptr) {
    vmem_putseg(vmp, vsp);
  }
   ...
   */
  p = (char *)vmem_alloc(vmem_seg_arena, size, vmflag & VM_UMFLAGS);
  if (p == nullptr) {
    lp->vmpl_thr = 0;
 ...
 */
static void vmem_advance(vmem_t *vmp, vmem_seg_t *walker, vmem_seg_t *afterme) {
  vmem_seg_t *vprev = walker->vs_aprev;
  vmem_seg_t *vnext = walker->vs_anext;
  vmem_seg_t *vsp = nullptr;

  VMEM_DELETE_a(walker);

  if (afterme != nullptr) {
    VMEM_INSERT_a(afterme, walker);
  }
   ...
   */
  if (vsp != nullptr && vsp->vs_import && vmp->vm_source_free != nullptr &&
 ...
 * all values in order.
 */
static void *vmem_nextfit_alloc(vmem_t *vmp, size_t size, int vmflag) {
  ...
  for (;;) {
    ...
    if (vsp == rotor) {
       ...
       */
      if (vmp->vm_source_alloc != nullptr || (vmflag & VM_NOSLEEP)) {
        (void)mutex_unlock(&vmp->vm_lock);
        return (vmem_xalloc(vmp, size, vmp->vm_quantum, 0, 0, nullptr, nullptr,
                            vmflag & VM_UMFLAGS));
      }
  ...
  ASSERT(vsp->vs_type == VMEM_ALLOC && vsp->vs_start == addr &&
         vsp->vs_end == addr + size);
...
void *vmem_xalloc(vmem_t *vmp, size_t size, size_t align, size_t phase,
  ...
  if (phase > 0 && phase >= align) {
    umem_panic("vmem_xalloc(%p, %lu, %lu, %lu, %lu, %p, %p, %x): "
               "invalid phase",
               (void *)vmp, size, align, phase, nocross, minaddr, maxaddr,
               vmflag);
  }
  ...
  for (;;) {
    ...
    if ((size & (size - 1)) == 0) {
      ...
      hb = highbit(size);
      if ((vmp->vm_freemap >> hb) == 0 || hb == VMEM_FREELISTS ||
    ...
    for (vbest = nullptr,
        vsp = (flist == 0) ? nullptr : vmp->vm_freelist[flist - 1].vs_knext;
         vsp != nullptr; vsp = vsp->vs_knext) {
      vmp->vm_kstat.vk_search++;
      if (vsp->vs_start == 0) {
        /*
         ...
         */
        flist = lowbit(P2ALIGN(vmp->vm_freemap, VS_SIZE(vsp)));
        if (flist-- == 0) {
          break;
        }
      ...
      start = std::max<size_t>(vsp->vs_start, (uintptr_t)minaddr);
      end = std::min<size_t>(vsp->vs_end - 1, (uintptr_t)maxaddr - 1) + 1;
      taddr = P2PHASEUP(start, align, phase);
      if (P2BOUNDARY(taddr, size, nocross)) {
        taddr += P2ROUNDUP(P2NPHASE(taddr, nocross), align);
      }
      if ((taddr - start) + size > end - start ||
          (vbest != nullptr && VS_SIZE(vsp) >= VS_SIZE(vbest))) {
        continue;
      }
      vbest = vsp;
      addr = taddr;
    ...
    if (vmp->vm_source_alloc != nullptr && nocross == 0 && minaddr == nullptr &&
        maxaddr == nullptr) {
      size_t asize = P2ROUNDUP(
          size + phase, std::max<size_t>(align, vmp->vm_source->vm_quantum));
       ...
       */
      resv = (size == asize)
                 ? VMEM_SEGS_PER_SPAN_CREATE + VMEM_SEGS_PER_EXACT_ALLOC
                 : VMEM_SEGS_PER_ALLOC_MAX;
      ASSERT(vmp->vm_nsegfree >= resv);
      vmp->vm_nsegfree -= resv; /* reserve our segs */
      (void)mutex_unlock(&vmp->vm_lock);
      vaddr = (Vaddr)vmp->vm_source_alloc(vmp->vm_source, asize,
                                          vmflag & VM_UMFLAGS);
      (void)mutex_lock(&vmp->vm_lock);
...
void vmem_xfree(vmem_t *vmp, void *vaddr, size_t size) {
   ...
   */
  if (vsp->vs_import && vmp->vm_source_free != nullptr &&
      vsp->vs_aprev->vs_type == VMEM_SPAN &&
      vsp->vs_anext->vs_type == VMEM_SPAN) {
 ...
 * on success, nullptr on failure.  vmflag specifies VM_SLEEP or VM_NOSLEEP,
...
void *vmem_alloc(vmem_t *vmp, size_t size, int vmflag) {
  ...
  if (size - 1 < vmp->vm_qcache_max) {
    ...
    return (_umem_cache_alloc(vmp->vm_qcache[(size - 1) >> vmp->vm_qshift],
                              UMEM_DEFAULT));
  }
  ...
  if (vmflag & (VM_BESTFIT | VM_FIRSTFIT)) {
    return (vmem_xalloc(vmp, size, vmp->vm_quantum, 0, 0, nullptr, nullptr, vmflag));
  }
...
void vmem_free(vmem_t *vmp, void *vaddr, size_t size) {
  if (size - 1 < vmp->vm_qcache_max) {
    _umem_cache_free(vmp->vm_qcache[(size - 1) >> vmp->vm_qshift], vaddr);
  } else {
 ...
 */
int vmem_contains(vmem_t *vmp, Vaddr vaddr, size_t size) {
  uintptr_t start = (uintptr_t)vaddr;
  uintptr_t end = start + size;
  vmem_seg_t *vsp;
 ...
 */
Vaddr vmem_add(vmem_t *vmp, Vaddr vaddr, size_t size, int vmflag) {
  if (vaddr == 0 || size == 0) {
    umem_panic("vmem_add(%p, %p, %lu): bad arguments", vmp, vaddr, size);
  }
 ...
 *      span.
 */
static vmem_seg_t *vmem_extend_unlocked(vmem_t *vmp, Vaddr addr,
                                        Vaddr endaddr) {
  vmem_seg_t *span;
  vmem_seg_t *vsp;
  ...
  if (span == end) {
    return (vmem_span_create(vmp, addr, endaddr - addr, 0));
  }
  if (span->vs_kprev->vs_end == addr && span->vs_start == endaddr) {
     ...
     */
    if (prevseg->vs_type == VMEM_FREE && nextseg->vs_type == VMEM_FREE) {
      /*
       * coalesce both ways
       */
      ASSERT(prevseg->vs_end == addr && nextseg->vs_start == endaddr);
 ...
 */
Vaddr _vmem_extend_alloc(vmem_t *vmp, Vaddr vaddr, size_t size, size_t alloc,
                         int vmflag) {
  uintptr_t addr = (uintptr_t)vaddr;
  uintptr_t endaddr = addr + size;
  vmem_seg_t *vsp;

  ASSERT(vaddr != 0 && size != 0 && endaddr > addr);
   ...
   */
  if (vmp->vm_source_alloc != nullptr) {
 ...
 * is adequate.
 */
void vmem_walk(vmem_t *vmp, int typemask, void (*func)(void *, void *, size_t),
               void *arg) {
  vmem_seg_t *vsp;
  vmem_seg_t *seg0 = &vmp->vm_seg0;
  vmem_seg_t walker;
  ...
  vmem_advance(vmp, &walker, nullptr);
  (void)mutex_unlock(&vmp->vm_lock);
}
...
size_t vmem_size(vmem_t *vmp, int typemask) {
  ...
  if (typemask & VMEM_FREE) {
    size += vmp->vm_kstat.vk_mem_total - vmp->vm_kstat.vk_mem_inuse;
  }
  return ((size_t)size);
}
 ...
 */
vmem_t *vmem_create(const char *name, Vaddr base, size_t size, size_t quantum,
  ...
  if (vmem_vmem_arena != nullptr) {
    vmp = (vmem_t *)vmem_alloc((vmem_t *)vmem_vmem_arena, sizeof(vmem_t),
                               vmflag & VM_UMFLAGS);
  } else {
  ...
  (void)mutex_init(&vmp->vm_lock, USYNC_THREAD, nullptr);
  (void)cond_init(&vmp->vm_cv, USYNC_THREAD, nullptr);
  vmp->vm_cflags = vmflag;
  vmflag &= VM_UMFLAGS;

  vmp->vm_quantum = quantum;
  vmp->vm_qshift = highbit(quantum) - 1;
  nqcache = std::min<size_t>(qcache_max >> vmp->vm_qshift, VMEM_NQCACHE_MAX);
  ...
  vmp->vm_freelist[0].vs_kprev = nullptr;
  vmp->vm_freelist[VMEM_FREELISTS].vs_knext = nullptr;
  ...
  if (nqcache != 0) {
    ...
    for (i = 0; i < nqcache; i++) {
      ...
      (void)snprintf(buf, sizeof(buf), "%s_%lu", vmp->vm_name,
                     (long)((i + 1) * quantum));
      vmp->vm_qcache[i] =
          umem_cache_create(buf, (i + 1) * quantum, quantum, nullptr, nullptr, nullptr,
                            nullptr, vmp, UMC_QCACHE | UMC_NOTOUCH);
      if (vmp->vm_qcache[i] == nullptr) {
  ...
  *vmpp = vmp;
  ...
  if ((base || size) && vmem_add(vmp, base, size, vmflag) == 0) {
    vmem_destroy(vmp);
    return (nullptr);
  }
...
void vmem_destroy(vmem_t *vmp) {
  ...
  *vmpp = vmp->vm_next;
  ...
  if (leaked != 0) {
    umem_printf("vmem_destroy('%s'): leaked %lu bytes", vmp->vm_name, leaked);
  }
 ...
 */
static void vmem_hash_rescale(vmem_t *vmp) {
  size_t nseg = (size_t)(vmp->vm_kstat.vk_alloc - vmp->vm_kstat.vk_free);

  size_t new_size =
      std::max<size_t>(VMEM_HASH_INITIAL, 1 << (highbit(3 * nseg + 4) - 2));
  size_t old_size = vmp->vm_hash_mask + 1;

  if ((old_size >> 1) <= new_size && new_size <= (old_size << 1)) {
    return;
  }

  vmem_seg_t **new_table = (vmem_seg_t **)vmem_alloc(
      vmem_hash_arena, new_size * sizeof(void *), VM_NOSLEEP);
  ...
  if (old_table != vmp->vm_hash0) {
    vmem_free(vmem_hash_arena, old_table, old_size * sizeof(void *));
  }
}
...
void vmem_update(void *dummy) {
  ...
  (void)mutex_lock(&vmem_list_lock);
  for (vmp = vmem_list; vmp != nullptr; vmp = vmp->vm_next) {
    /*
...
void vmem_startup(void) {
  ...
  vmem_id = 0;
  vmem_populators = 0;
  vmem_segfree = nullptr;
  vmem_list = nullptr;
  vmem_internal_arena = nullptr;
  vmem_seg_arena = nullptr;
  vmem_hash_arena = nullptr;
  vmem_vmem_arena = nullptr;
  vmem_heap = nullptr;
  vmem_heap_alloc = nullptr;
  vmem_heap_free = nullptr;
...
vmem_t *vmem_init(const char *parent_name, size_t parent_quantum,
                  ...
                  size_t heap_quantum, vmem_alloc_t *heap_alloc,
                  vmem_free_t *heap_free) {
  uint32_t id;
  int nseg = VMEM_SEG_INITIAL;
  vmem_t *parent, *heap;

  ASSERT(vmem_internal_arena == nullptr);
  ...
  if (parent_name != nullptr) {
    parent =
        vmem_create(parent_name, (Vaddr)heap_start, heap_size, parent_quantum,
                    nullptr, nullptr, nullptr, 0, VM_SLEEP | VMC_POPULATOR);
    heap_start = nullptr;
    heap_size = 0;
  } else {
    ASSERT(parent_alloc == nullptr && parent_free == nullptr);
    parent = nullptr;
  }

  heap = vmem_create(heap_name, (Vaddr)heap_start, heap_size, heap_quantum,
  ...
  vmem_internal_arena =
      vmem_create("vmem_internal", 0, 0, heap_quantum, heap_alloc, heap_free,
                  heap, 0, VM_SLEEP | VMC_POPULATOR);

  vmem_seg_arena =
      vmem_create("vmem_seg", 0, 0, heap_quantum, vmem_alloc, vmem_free,
                  vmem_internal_arena, 0, VM_SLEEP | VMC_POPULATOR);

  vmem_hash_arena = vmem_create("vmem_hash", 0, 0, 8, vmem_alloc, vmem_free,
                                vmem_internal_arena, 0, VM_SLEEP);

  vmem_vmem_arena =
      vmem_create("vmem_vmem", (Vaddr)vmem0, sizeof(vmem0), 1, vmem_alloc,
                  vmem_free, vmem_internal_arena, 0, VM_SLEEP);

  for (id = 0; id < vmem_id; id++) {
    (void)vmem_xalloc(vmem_vmem_arena, sizeof(vmem_t), 1, 0, 0, &vmem0[id],
                      &vmem0[id + 1], VM_NOSLEEP | VM_BESTFIT | VM_PANIC);
  }
...
void vmem_no_debug(void) {
   ...
   */
  vmem_seg_size = P2ROUNDUP(offsetof(vmem_seg_t, vs_thread), sizeof(hrtime_t));
}
...
void vmem_lockup(void) {
   ...
   */
  for (cur = vmem_list; cur != nullptr; cur = cur->vm_next) {
