...
#ifndef _WIN32
  if (ret != NULL && mmap(ret, size, ALLOC_PROT, ALLOC_FLAGS | MAP_FIXED, -1,
                          0) == MAP_FAILED) {
...
vmem_t *vmem_mmap_arena(vmem_alloc_t **a_out, vmem_free_t **f_out) {
  ...
  if (mmap_heap == NULL) {
    mmap_heap = vmem_init("mmap_top", CHUNKSIZE, vmem_mmap_top_alloc, vmem_free,
                          "mmap_heap", NULL, 0, pagesize, vmem_mmap_alloc,
                          vmem_mmap_free);
  }
