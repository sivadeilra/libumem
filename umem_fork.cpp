...
static void umem_lockup_cache(umem_cache_t *cp) {
  int idx;
...
static void umem_release_cache(umem_cache_t *cp) {
  int idx;
  int ncpus = cp->cache_cpu_mask + 1;
...
static void umem_lockup_log_header(umem_log_header_t *lhp) {
  int idx;
  if (lhp == NULL)
    return;
...
static void umem_release_log_header(umem_log_header_t *lhp) {
  int idx;
  if (lhp == NULL)
    return;
  ...
  if (cleanup_update) {
    ...
    for (cp = umem_null_cache.cache_next; cp != &umem_null_cache;
      ...
      if (cp->cache_uflags & UMU_ACTIVE) {
        ...
        ASSERT(cp->cache_unext == NULL && cp->cache_uprev == NULL);
...
static void umem_release(void) { umem_do_release(0); }

static void umem_release_child(void) { umem_do_release(1); }
#endif
